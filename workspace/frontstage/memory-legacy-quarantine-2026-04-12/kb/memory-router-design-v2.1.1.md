...
>
> 当前 `workbot` 的正式 global canonical 以 [workspace/memory/kb/global/INDEX.md](/Users/busiji/workbot/workspace/memory/kb/global/INDEX.md) 的 “Current Local Canonical” 为准。
>
...
- 追加写入当日 `workspace/memory/log/YYYY-MM-DD.md`
...
**桥接规则（必须）**：
- 若 `workspace/projects/**` 中存在长文档（交付件/报告），允许保留
- 但必须在文件顶部加一行指针（不改正文）：
...
- ✅ **索引核心范围（必须）**：`workspace/memory/docs/**` + `workspace/memory/kb/**`
- 🟡 **允许附带（可选）**：`workspace/memory/log/**`、`workspace/memory/reflections/**`、`workspace/memory/system/**`、`workspace/memory/short-index.md`
- ❌ **禁止索引范围（硬禁）**：`archive-memory/**`、任何 `.archive/**`、`workspace/projects/**`
...
**目录**：`workspace/memory/reflections/**`
...
`workspace/memory/kb/**` 必须包含 YAML frontmatter，用于治理与可信度：
