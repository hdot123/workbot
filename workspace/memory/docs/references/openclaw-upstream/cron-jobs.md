...
* One-shot (`at`) jobs retry transient errors (rate limit, network, server\_error) up to 3 times with backoff; permanent errors disable immediately. See [Retry policy](/automation/cron-jobs#retry-policy).
