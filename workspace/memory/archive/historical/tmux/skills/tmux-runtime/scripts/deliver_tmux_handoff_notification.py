...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Deliver a tmux-runtime handoff notification to the CODEX_THREAD_ID-bound Codex window thread."
    )
    parser.add_argument(
        "--event-file",
    ...
    parser.add_argument(
        "--bundle-file",
        help="Optional path to a prebuilt tmux-runtime handoff bundle. If omitted, accepts an event or bundle on stdin.",
    )
    parser.add_argument(
        "--session-mode",
        choices=("fixed", "new"),
...
def main() -> int:
    ...
    try:
        ...
        tmux_handoff = bundle["tmux_runtime_handoff"]
        ...
        if not message:
            raise ValueError("tmux_runtime_handoff.notification.message is empty")
        if not codex_thread_id:
            raise ValueError("CODEX_THREAD_ID is missing from tmux-runtime handoff payload")
