...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a tmux-skills handoff bundle from one tmux notification event."
    )
    parser.add_argument(
        "--event-file",
    ...
    parser.add_argument(
        ...
        choices=("fixed", "new"),
        default="fixed",
        help="Codex session mode for tmux-skills handoff delivery, defaults to fixed",
    )
...
def build_bundle(event: dict[str, Any], *, table: str, session_mode: str) -> dict[str, Any]:
    ...
    return {
        ...
        "event_id": event_id,
        "session_mode": session_mode,
        "tmux_skills_handoff": tmux_skills_handoff,
        "db_write": db_write,
        "fanout": [
            {"channel": "tmux_skills_handoff", "payload": tmux_skills_handoff},
