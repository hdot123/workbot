# node-11 多租客 OpenClaw 环境部署文档 (Ubuntu 24.04 版本)

**部署日期**: 2026-03-02
**服务器**: node-11 (192.168.88.30)
...
### 2. 安装 Mise (Node.js 版本管理器)

```bash
# 安装 Mise
curl https://mise.run | sh

# 添加到 PATH (Ubuntu 使用 .bashrc)
...
### 7. 安装 Caddy (Ubuntu 24.04)

```bash
# 安装必要的依赖
apt install -y debian-keyring debian-archive-keyring apt-transport-https curl

# 添加 Caddy 源 (Ubuntu 24.04 noble)
curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/gpg.key' | gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg
curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt' | tee /etc/apt/sources.list.d/caddy-stable.list

# 注意：Ubuntu 24.04 使用 noble，但 Caddy 可能还没有 noble 的包
# 如果上面的源不可用，可以尝试使用 bookworm (Debian 12) 的包
# curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt' | sed 's/bookworm/noble/g' | tee /etc/apt/sources.list.d/caddy-stable.list
...
**访问地址**：
- **user1**: `http://192.168.88.30:19010/`
- **user2**: `http://192.168.88.30:19050/`
- **user3**: `http://192.168.88.30:19090/`
...
## 🔄 Ubuntu 24.04 vs Debian 13 差异

### 1. 包管理器

- **Debian**: 使用 `.deb` 包，源配置在 `/etc/apt/sources.list`
- **Ubuntu**: 使用 `.deb` 包，源配置在 `/etc/apt/sources.list.d/`
...
- **Debian**: 使用 Debian 源
- **Ubuntu**: 可能需要使用兼容的源（如 bookworm）或等待官方 noble 支持

### 4. 系统服务

- **Debian**: 使用 systemd
- **Ubuntu**: 使用 systemd（相同）

---

**文档版本**: 1.0 (Ubuntu 24.04)
**最后更新**: 2026-03-02
**维护者**: Molt (战术蟹王)
**基于**: node-11_multi_tenant_deployment.md (Debian 13 版本)
