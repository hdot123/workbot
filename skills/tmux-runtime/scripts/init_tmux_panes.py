#!/usr/bin/env python3
"""Apply pane titles for tmux-runtime without any Claude/scene validation."""
...
def require_visible_formal_client(snapshot: dict[str, Any], formal_session: str) -> None:
    ...
    if not current_client.get("inside_tmux"):
        raise RuntimeError("tmux-runtime positive flow requires current client to be inside tmux")
