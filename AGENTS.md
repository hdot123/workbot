...
- tmux stage roles (for runtime stages such as task/monitor/runtime slots) belong only to the `tmux-skills` domain (`/Users/busiji/workbot/skills/tmux-skills/`) and are not project-level Claude identities.
- Do not define, mirror, or migrate tmux stage roles into `/Users/busiji/workbot/.claude/agents/`.
...
- tmux stage-role definitions and stage lifecycle rules are owned only by `tmux-skills`; `.claude/agents/` must not be used as their source of truth.
...
- `task` / `monitor` / `runtime` and other stage roles must exist as pane or slot roles inside that one formal session, not as separate formal sessions.
...
- The tmux doorbell pipeline must use the `CODEX_THREAD_ID` injected by the active monitor pane / slot as its only delivery target. Any other candidate variable or fallback thread constant is deprecated; the doorbell should fail fast when `CODEX_THREAD_ID` is unset.
