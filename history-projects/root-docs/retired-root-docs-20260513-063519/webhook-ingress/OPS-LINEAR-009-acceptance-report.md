...
- Business action was decoupled from ingress core handler:
  - `workspace/tools/webhook_ingress/actions.py`: `ActionRegistry`, `LinearCanaryCommentAction`, label guard.
  - `workspace/tools/webhook_ingress/executors.py`: `LinearCanaryCommentExecutor` for the Linear `commentCreate` side effect.
