...
> RETIRED HISTORICAL CONFLICT LEDGER
> 本文件来自旧体系的历史冲突登记，只保留证据价值；它不是当前 `workbot` 的 active conflict queue，也不是合法 canonical。
>
> 当前冲突若需要进入正式系统，必须通过 `project-map`、`kb/decisions/**`、`kb/lessons/**` 或其他现行 canonical 重新登记与裁决。
>
> 用途：记录发现的"同一字段多版本"冲突项，等待人类裁决
> 创建日期：2026-03-02
> 维护者：Molt King
> 状态：retired historical ledger
