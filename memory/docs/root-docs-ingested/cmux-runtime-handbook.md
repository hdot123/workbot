...
- assignment 由 `generate_cmux_assignments.py` 生成/同步到：
  - `/Users/busiji/workbot/artifacts/cmux-runtime/cmux-assignment.json`
...
- 启动完成后会产出运行清单：
  - `artifacts/cmux-runtime/runtime-launch-manifest-<bot>.json`
...
- `.claude/settings.local.json` 必须配置四个事件：
  ...
  - `Stop`
  - `Notification`
- 缺 `surface/state_file` 时必须 fail-close（返回码 `2`，`missing_hook_context`），不得静默成功。
- Hook 状态统一写入 runtime state 文件（默认在 `artifacts/cmux-runtime/`）。
...
- watcher 持续输出 consumer sidecar：
  - `artifacts/cmux-runtime/cmux-consumer-state-latest.json`

统一健康检查：

```bash
python3 /Users/busiji/.agents/skills/cmux/scripts/cmux_runtime_ctl.py status \
  --assignment-file /Users/busiji/workbot/artifacts/cmux-runtime/cmux-assignment.json
```
...
- `A1-A9` 语义仍有效，但 `cmux` 当前 assignment 真源不是历史 `lookme-assignment.json`，而是 `artifacts/cmux-runtime/cmux-assignment.json`。
