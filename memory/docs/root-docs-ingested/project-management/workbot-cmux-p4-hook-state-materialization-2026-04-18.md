...
- 因此 `cmux_claude_hook_bridge.py` 不会稳定接到 live runtime 事件，`/Users/busiji/workbot/artifacts/cmux-runtime/hook-state.json` 虽然存在，但会停留在空的 `surfaces` 或只看到部分事件。
...
- live state file:
  - `/Users/busiji/workbot/artifacts/cmux-runtime/hook-state.json`
...
cmux tree
sed -n '1,160p' /Users/busiji/workbot/artifacts/cmux-runtime/hook-state.json
```
